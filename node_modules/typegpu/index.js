...
`;return n}}};function qt(e,t,r,n=0){let a=["i","j","k"][n]||`i${n}`;if(yn(e)||ct(e))return qt(e.inner,t,r,n);if(_(e)||re(e)){let s=Ot(e),i="";for(let[u,l]of Object.entries(s)){let m=e.propTypes[u];m&&(i+=qt(m,`(${t} + ${l.offset})`,`${r}.${u}`,n))}return i}if(z(e)||gt(e)){let s=M(I(e.elementType),$(e)),i="";return i+=`for (let ${a} = 0; ${a} < ${e.elementCount}; ${a}++) {
...
`;return i}if(gn(e)){let s=zt[e.type],i=Nt[s],u=dn(e)?2:cn(e)?3:4,l=u*u,m=M(u*4,8),y="";for(let f=0;f<l;f++){let T=Math.floor(f/u),w=f%u,x=T*m+w*4;y+=`output.${i}((${t} + ${x}), ${r}.columns[${T}].${["x","y","z","w"][w]}, littleEndian);
